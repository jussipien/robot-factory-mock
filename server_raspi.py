...
server_start_time = time.mktime(datetime.datetime.now().timetuple())
...
def OEE_builder(builder):
    ...
    c.close()
    start_timestamp = dateutil.parser.parse(get_time_to_startup(builder))
    server_timestamp = datetime.datetime.now()
    seconds1 = time.mktime(start_timestamp.timetuple()) - server_start_time
    seconds2 = time.mktime(server_timestamp.timetuple()) - server_start_time
    print(seconds1)
    print(seconds2)
    return render_template('chartview.html', builder=builder, perfect=perfect, good=good, unusable=unusable,
                           seconds1=seconds1, seconds2=seconds2)
...
def get_time_to_startup(builder):
    c = conn.cursor()
    builder_s = "'" + builder + "'"
    c.execute('''SELECT time FROM events WHERE builder = ''' + builder_s + ''' AND type = "START"
